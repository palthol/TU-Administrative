...
import { MarketingLeadsTab } from '@/components/admin/MarketingLeadsTab';
import { NotificationsTab } from '@/components/admin/NotificationsTab';
import { OperatingExpensesTab } from '@/components/admin/OperatingExpensesTab';
import { ParticipantSearchTab } from '@/components/admin/ParticipantSearchTab';
import { PerClassTab } from '@/components/admin/PerClassTab';
import { SchedulingTab } from '@/components/admin/SchedulingTab';
import { SubscriptionCreateTab } from '@/components/admin/SubscriptionCreateTab';
import { WaiverTab } from '@/components/admin/WaiverTab';
...
const ADMIN_PAGES: { page: AdminPage; label: string; hint: string }[] = [
  { page: 'participant-search', label: 'Participant search', hint: 'Lookup participants and linked accounts.' },
  { page: 'merge', label: 'Merge participants', hint: 'Combine duplicate participant records.' },
  { page: 'subscription', label: 'Create subscription', hint: 'Enroll a participant on a plan.' },
  { page: 'upgrade', label: 'Plan upgrade', hint: 'Prorated subscription upgrade charge.' },
  { page: 'per-class', label: 'Pay-per-class', hint: 'Charge from attendance or upgrade to monthly.' },
  ...
  { page: 'waiver', label: 'Waivers', hint: 'List waivers and fetch signed PDF/signature URLs.' },
];

const OPERATIONS_PAGES: { page: AdminPage; label: string; hint: string }[] = [
  { page: 'expenses', label: 'Operating expenses', hint: 'Record and list monthly operating costs.' },
  { page: 'marketing-leads', label: 'Marketing leads', hint: 'Trial inquiries from the public site.' },
  { page: 'notifications', label: 'Discord notifications', hint: 'Post payment reminders or daily digest.' },
  { page: 'scheduling', label: 'Scheduling', hint: 'Sessions list, create, cancel, and attendance.' },
];
  ...
  return (
    <aside className={asideClass}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        <p className="mt-6 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Operations</p>
        <nav className="mt-2 flex flex-col gap-0.5" aria-label="Operations">
          {OPERATIONS_PAGES.map((a) => (
            <NavButton
              key={a.page}
              active={nav.kind === 'admin' && nav.page === a.page}
              onClick={() => pick({ kind: 'admin', page: a.page })}
...
export default function App() {
  ...
  const currentAdminMeta =
    nav.kind === 'admin'
      ? [...ADMIN_PAGES, ...OPERATIONS_PAGES].find((a) => a.page === nav.page)
      : undefined;
        ...
        <main className="min-w-0 flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
          ...
          {nav.kind === 'admin' && currentAdminMeta && (
            <div className="mx-auto max-w-2xl space-y-2">
              ...
              {nav.page === 'participant-search' && (
                <ParticipantSearchTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />
              )}
              {nav.page === 'merge' && <MergeTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />}
              {nav.page === 'subscription' && (
                <SubscriptionCreateTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />
              )}
              {nav.page === 'upgrade' && <UpgradeTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />}
              {nav.page === 'per-class' && <PerClassTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />}
              ...
              {nav.page === 'expenses' && (
                <OperatingExpensesTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />
              )}
              {nav.page === 'marketing-leads' && (
                <MarketingLeadsTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />
              )}
              {nav.page === 'notifications' && (
                <NotificationsTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />
              )}
              {nav.page === 'scheduling' && (
                <SchedulingTab apiBase={apiBase} adminKey={adminKey} requireKey={requireKey} />
              )}
            </div>
          )}
        </main>
